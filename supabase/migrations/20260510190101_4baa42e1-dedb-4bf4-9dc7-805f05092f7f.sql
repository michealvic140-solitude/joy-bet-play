...
CREATE TYPE public.app_role AS ENUM ('viewer','shooter','gang_leader','registered','moderator','admin','sponsor');
CREATE TYPE public.gang_type AS ENUM ('G','F');
CREATE TYPE public.match_status AS ENUM ('scheduled','live','ended','cancelled');
CREATE TYPE public.bet_status AS ENUM ('open','won','lost','cashed_out','void','suspended','refunded');
...
CREATE TABLE public.profiles (
  ...
  full_name TEXT NOT NULL, email TEXT NOT NULL, phone TEXT, discord_username TEXT, discord_full_name TEXT, ingame_name TEXT,
...
CREATE OR REPLACE FUNCTION public.create_withdrawal_request(_amount bigint, _ingame text, _gang text, _ticket text DEFAULT NULL)
...
BEGIN
  ...
  INSERT INTO notifications(user_id, title, body) VALUES (uid, 'Withdrawal requested', 'Your request for '||_amount||' tokens has been submitted.');
  RETURN req_id;
END $$;

CREATE OR REPLACE FUNCTION public.review_withdrawal_request(_id uuid, _approve boolean, _note text DEFAULT NULL)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE r record;
BEGIN
  ...
  IF _approve THEN
    ...
    INSERT INTO notifications(user_id, title, body) VALUES (r.user_id, 'Withdrawal approved', COALESCE(_note,'Your withdrawal of '||r.amount||' tokens has been approved.'));
  ELSE
    ...
    INSERT INTO notifications(user_id, title, body) VALUES (r.user_id, 'Withdrawal declined', COALESCE(_note,'Your withdrawal was declined. Tokens have been refunded.'));
  END IF;
END $$;
...
BEGIN
  ...
  SELECT COUNT(*) INTO existing_count FROM public.bet_selections bs JOIN public.bets b ON b.id = bs.bet_id
  WHERE bs.match_id = NEW.match_id AND b.user_id = uid AND b.status IN ('open','suspended') AND bs.bet_id <> NEW.bet_id;
  IF existing_count > 0 THEN RAISE EXCEPTION 'You already have an active ticket on this match.'; END IF;
  RETURN NEW;
END $$;
...
BEGIN
  ...
  INSERT INTO public.promo_codes(code, amount, usage_limit, used_count, is_active, created_by) VALUES (new_code, r.amount, r.usage_limit, 0, true, auth.uid()) RETURNING id INTO new_promo;
  ...
  INSERT INTO public.notifications(user_id, title, body, link) VALUES (r.user_id, 'Promo code approved', 'Your promo code request was approved. Code: '||new_code, '/dashboard');
  RETURN new_promo;
END $$;
...
BEGIN
  ...
  INSERT INTO public.notifications(user_id, title, body) VALUES (r.user_id, 'Promo code request declined', COALESCE(_note,'Your promo code request was declined.'));
END $$;
...
BEGIN
  ...
  INSERT INTO public.notifications(user_id, title, body, link) VALUES (b.user_id, 'Ticket suspended', COALESCE(_reason,'Your bet ticket has been suspended.'), '/ticket/'||_bet_id);
  INSERT INTO public.audit_logs(actor_id, action, target_type, target_id, metadata) VALUES (auth.uid(), 'suspend_bet', 'bet', _bet_id::text, jsonb_build_object('reason', _reason));
END $$;
...
BEGIN
  ...
  INSERT INTO public.audit_logs(actor_id, action, target_type, target_id) VALUES (auth.uid(), 'unsuspend_bet', 'bet', _bet_id::text);
END $$;
...
BEGIN
  ...
  INSERT INTO public.notifications(user_id, title, body) VALUES (b.user_id, 'Ticket removed', COALESCE(_reason,'Your bet ticket has been removed.') || CASE WHEN _refund THEN ' Stake refunded.' ELSE '' END);
  INSERT INTO public.audit_logs(actor_id, action, target_type, target_id, metadata) VALUES (auth.uid(), 'delete_bet', 'bet', _bet_id::text, jsonb_build_object('reason', _reason, 'refunded', _refund, 'stake', b.stake));
END $$;
...
BEGIN
  ...
  SELECT * INTO b FROM public.bets WHERE id = _bet_id FOR UPDATE;
  IF b IS NULL THEN RAISE EXCEPTION 'Bet not found'; END IF;
  IF b.status = 'refunded' THEN RAISE EXCEPTION 'Already refunded'; END IF;
  IF b.status IN ('won','cashed_out') THEN RAISE EXCEPTION 'Cannot refund paid ticket'; END IF;
  ...
  INSERT INTO public.notifications(user_id, title, body, link) VALUES (b.user_id, 'Ticket refunded', COALESCE(_reason,'Your bet ticket stake has been refunded.'), '/ticket/'||_bet_id);
  INSERT INTO public.audit_logs(actor_id, action, target_type, target_id, metadata) VALUES (auth.uid(), 'refund_bet', 'bet', _bet_id::text, jsonb_build_object('reason', _reason, 'stake', b.stake));
END $$;
...
BEGIN
  ...
  SELECT * INTO b FROM public.bets WHERE id = _bet_id FOR UPDATE;
  IF b IS NULL THEN RAISE EXCEPTION 'Bet not found'; END IF;
  IF b.status IN ('won','cashed_out','refunded') THEN RAISE EXCEPTION 'Cannot void paid/refunded ticket'; END IF;
  IF _refund THEN UPDATE public.profiles SET token_balance = token_balance + b.stake WHERE id = b.user_id; END IF;
  UPDATE public.bets SET status = 'void', settled_at = now() WHERE id = _bet_id;
  INSERT INTO public.notifications(user_id, title, body, link) VALUES (b.user_id, CASE WHEN _refund THEN 'Ticket voided and refunded' ELSE 'Ticket voided' END, COALESCE(_reason, 'Your bet ticket has been voided.') || CASE WHEN _refund THEN ' Stake refunded.' ELSE '' END, '/ticket/' || _bet_id);
  INSERT INTO public.audit_logs(actor_id, action, target_type, target_id, metadata) VALUES (auth.uid(), 'void_bet', 'bet', _bet_id::text, jsonb_build_object('reason', _reason, 'refunded', _refund, 'stake', b.stake));
END $$;

CREATE TABLE public.user_tasks (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL, title text NOT NULL, description text,
  reward_tokens bigint NOT NULL DEFAULT 0, status text NOT NULL DEFAULT 'pending',
...
CREATE TABLE public.user_achievements (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL, code text NOT NULL, title text NOT NULL,
  description text, icon text, awarded_at timestamptz NOT NULL DEFAULT now()
);
...
BEGIN
  FOR t IN SELECT unnest(ARRAY[
    ...
    'ban_appeals','withdrawal_requests','promo_code_requests'
  ]) LOOP
