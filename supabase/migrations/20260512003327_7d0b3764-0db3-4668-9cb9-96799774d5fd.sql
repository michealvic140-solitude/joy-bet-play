...
BEGIN
  ...
  END IF;
  IF p.max_uses IS NOT NULL AND p.used_count >= p.max_uses THEN RAISE EXCEPTION 'This code has reached its maximum redemptions'; END IF;
  SELECT count(*) INTO user_uses FROM public.promo_redemptions WHERE promo_id = p.id AND user_id = uid;
  IF user_uses >= COALESCE(p.usage_limit, 1) THEN RAISE EXCEPTION 'You have already used this code the maximum number of times'; END IF;
  ...
  INSERT INTO public.token_transactions(user_id, amount, balance_after, kind, description, metadata) VALUES (uid, p.amount, new_balance, 'promo', 'Promo code: ' || p.code, jsonb_build_object('promo_id', p.id, 'code', p.code));
  INSERT INTO public.notifications(user_id, title, body) VALUES (uid, 'Promo redeemed', '+' || p.amount || ' tokens from code ' || p.code);
